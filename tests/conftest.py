...
def wait_for_database(timeout_seconds=60):
    ...
    while time.time() - start_time < timeout_seconds:
        try:
            ...
            with psycopg2.connect(**config):
                print(
                    f"✓ Database connection successful: {config['host']}:{config['port']}"
                )
                return True
    ...
    raise RuntimeError(
        f"Database not ready after {timeout_seconds}s. Last error: {last_error}"
    )
...
def run_migrations():
    ...
    result = subprocess.run(
        cmd, cwd=repo_root, capture_output=True, text=True, env=os.environ.copy()
    )
