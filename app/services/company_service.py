...
class CompanyService:
    ...
    def _get_session(self) -> Session:
        ...
        if self._db_session:
            ...
            raise RuntimeError(
                "No database session provided. Use with get_db_session()."
            )
    ...
    def get_company_by_companies_house_number(
        self, ch_number: str
    ) -> Optional[Company]:
        """
    ...
    def get_companies_by_postcode(self, postcode: str) -> List[Company]:
        ...
        query = (
            select(Company)
            .where(Company.postcode.ilike(postcode_pattern))
    ...
    def update_from_companies_house(
        self, company_id: int, ch_data: Dict[str, Any]
    ) -> Company:
        """
    ...
    def _build_filter_conditions(self, filters: CompanyFilter) -> List:
        ...
        if filters.industry:
            conditions.append(
                func.lower(Company.industry).contains(filters.industry.lower())
            )
...
def list_companies_with_session(
    filters: Optional[CompanyFilter] = None, limit: int = 50, offset: int = 0
) -> tuple[List[Company], int]:
